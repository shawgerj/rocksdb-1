...
int perfPutNormal(int records, int value_size) {
    DB* db;
    Status s = DB::Open(dbOptions, kDBPath, &db);
...
int main() {
    ...
    assert(perfPutNormal(10000) == 1);
    rocksdb::SetPerfLevel(rocksdb::PerfLevel::kDisable);
    PerfContext* ctx = get_perf_context();

    std::cout << "write_wal_time: " << ctx->write_wal_time << std::endl;
    std::cout << "write_memtable_time: " << ctx->write_memtable_time << std::endl;
    std::cout << "write_thread_wait_nanos: " <<
        ctx->write_thread_wait_nanos << std::endl;
